...
const site = () => {
  ...
  switch (URL) {
    case "index.html":
      printCard(URL, arrHome, cardsContainer);
      break;
    case "upcomingEvents.html":
      printCard(URL, arrUpcoming, upcomingEvents);
      break;
    case "pastEvents.html":
      printCard(URL, arrPastEvents, pastEvents);
      break;
  }
};
...
const printCard = (URL, array, container) => {
  ...
  if (URL === URLORDER) {
    array.map((e) => {
      container.insertAdjacentHTML("beforeend", e);
    });
  }
};
...
const categoriesGenerator = () => {
  ...
  const categoryCheckbox = category.map(
    (category) => `<div class="form-check me-2 ms-2">
                                                        <input class="form-check-input" type="checkbox" id="info" name="category" value="${category}">
                                                          ${category}
                                                      </div>`
  );

  categoryCheckbox.map((e) => {
    categoriesBar.insertAdjacentHTML("beforeend", e);
  });
...
const searchAndFilterCards = () => {
  ...
  search.addEventListener("keyup", (event) => {
    ...
    for (let i = 0; i < elemntsCategory.length; i++) {
      if (
        elemntsCategory[i].parentElement.children[0].textContent
          .toLowerCase()
          .indexOf(inputText) === -1
      ) {
        elemntsCategory[i].parentElement.parentElement.classList.add("d-none");
    ...
    const cardsNotHidden =
      document.getElementsByClassName("card d-none").length;
